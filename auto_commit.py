...
DEFAULT_BRANCH = "main" # Change to "master" if your repo uses master
...
def human_behavior_decision():
    """الگوریتم تصمیم‌گیری با در نظر گرفتن تعطیلات"""
...
def generate_working_hours_times(num_commits):
    ...
    # FIX 1: اگر اسکریپت قبل از شروع ساعت کاری اجرا شد، ساعت کاری را به عقب بکش
    if now < start_work:
        start_work = now - datetime.timedelta(hours=2) # اطمینان از زمان کافی
        end_work = now
    ...
    while i < num_commits:
        ...
        if random.randint(1, 100) <= 15 and i + 1 < num_commits:
            ...
            times.append((quick_fix_time, True))
            i += 1
...
def manage_issues():
    ...
    if random.randint(1, 100) <= 60:
        ...
        try:
            ...
            print(f"Warning: Could not fetch/close issues (gh cli might not be configured).")
...
def manage_pull_requests_and_releases(unique_id):
    if random.randint(1, 100) <= 15:
        ...
        try:
            ...
            # ایجاد Pull Request
            run_command(["gh", "pr", "create", "--title", pr_title, "--body", "Automated PR created to fix the issue.", "--head", branch_name, "--base", DEFAULT_BRANCH])
            time.sleep(3) 
    ...
    if random.randint(1, 100) <= 5:
        version = f"v1.0.{random.randint(10,99)}" # FIX 3: Realistic versioning
        try:
...
def main():
    # اگر گیت‌هاب اکشنز یا سیستم اتوماسیون ایمیلی تنظیم نکرده، آن را تنظیم کن
    try:
        subprocess.run(["git", "config", "--global", "user.email"], check=True, capture_output=True)
    except subprocess.CalledProcessError:
        print("Git user not configured. Please run git config user.email and user.name")

    # بررسی آپدیت بودن برنچ قبل از شروع
    try:
        run_command(["git", "pull", "origin", DEFAULT_BRANCH])
    except Exception:
        print(f"Warning: Could not pull from origin {DEFAULT_BRANCH}. Make sure you are connected.")
    ...
    if not will_work:
        # برای اینکه گراف کاملاً خالی نماند، اگر اسکریپت از طریق کرون‌جاب ران می‌شود
        # و کاربر 'مرخصی' است، فقط اسکریپت را می‌بندیم. 
        # هیچ کامیتی پوش نمی‌شود.
        sys.exit(0)
    ...
    # FIX 2: Create ALL commits first locally
    for i in range(num_commits):
        ...
        with open(FILE_NAME, "a") as file:
            # ذخیره کردن تاریخ واقعی برای دیباگ بهتر
            file.write(f"Contribution logged for simulated time {formatted_time} (Hash: {current_unique_id})\n")
        
        run_command(["git", "add", FILE_NAME])
        run_command(["git", "commit", "-m", full_message], env=env_vars)
        
        print(f"Committed {i+1}/{num_commits} for time: {formatted_time}")
        time.sleep(0.5) # کمی مکث بین کامیت‌ها
        
    # FIX 4: Push ALL commits at the end of the script
    print("Pushing all commits to origin...")
    try:
        run_command(["git", "push", "origin", DEFAULT_BRANCH])
        print("Workday finished and pushed successfully.")
    except Exception as e:
        print("Error during push. Commits are saved locally.")
